    ...
    def generate_map(vehicle_data, locations):
        ...
        for gene in vehicle_data[2]:
            ...
            folium.Marker(
                ...
                icon=DivIcon(
                    icon_size=(30, 30),
                    icon_anchor=(15, 30),  # Menyesuaikan posisi ikon
                    html=f'<div style="font-size: 14pt; color: white; background-color: blue; border-radius: 50%; text-align: center; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">{gene[0]}</div>'
                )
            ).add_to(m)
            ...
            folium.Marker(
                ...
                icon=DivIcon(
                    icon_size=(30, 30),
                    icon_anchor=(15, 30),
                    html=f'<div style="font-size: 14pt; color: white; background-color: red; border-radius: 50%; text-align: center; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">{gene[1]}</div>'
                )
            ).add_to(m)
